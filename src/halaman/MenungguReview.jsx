      ...
      <div className="ls-body">
        ...
        <main className="ls-main">
          ...
          <div className="ls-tabs">
            {TABS.map(tab => (
              <Link key={tab.id} to={tab.to}
                className={`ls-tab ${tab.id === 'review' ? 'ls-tab-aktif' : ''}`}
          ...
          <div className="ds-info-bar">
            <span className="ds-total"><strong>{reviewedCount}</strong> lamaran sedang ditinjau</span>
          </div>

          {loading ? (
            <div className="text-center py-10 text-gray-500">Memuat data...</div>
          ) : reviewedApps.length === 0 ? (
            <div className="text-center py-10 text-gray-500">Belum ada lamaran yang sedang ditinjau.</div>
          ) : (
                ...
                <div key={app.id} className="lt-card">
                  <div className="lt-card-left">
                    ...
                    <div className="lt-info">
                      ...
                      <p className="lt-tanggal">Dilamar pada {new Date(app.created_at).toLocaleDateString('id-ID')} · Menunggu review tim</p>
                    </div>
                  </div>

                  {/* Progress Steps (reviewed = step ke-2) */}
                  <div className="lt-progress">
                    {stepLabels.map((label, i) => (
                      <div key={i} className="lt-step-wrap">
                        ...
                        <span className={`lt-step-label ${i <= 1 ? 'lt-label-aktif' : ''}`}>{label}</span>
                        {i < stepLabels.length - 1 && (
                          <div className={`lt-step-line ${i < 1 ? 'lt-line-aktif' : ''}`} />
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="lt-card-right">
                    <span className="ls-badge ls-badge-ditinjau">
