      ...
      <div className="ls-body">
        ...
        <main className="ls-main">
          ...
          <div className="ls-tabs">
            {TABS.map(tab => (
              <Link key={tab.id} to={tab.to}
                className={`ls-tab ${tab.id === 'wawancara' ? 'ls-tab-aktif' : ''}`}
          ...
          <div className="ds-info-bar">
            <span className="ds-total"><strong>{acceptedCount}</strong> undangan wawancara</span>
          </div>

          {loading ? (
            <div className="text-center py-10 text-gray-500">Memuat data...</div>
          ) : acceptedApps.length === 0 ? (
            <div className="text-center py-10 text-gray-500">Belum ada undangan wawancara.</div>
          ) : (
              ...
              {acceptedApps.map(app => (
                <div key={app.id} className="lt-card" style={{ border: '1.5px solid #bfdbfe', background: '#eff6ff' }}>
                  <div className="lt-card-left">
                    ...
                    <div className="lt-info">
                      ...
                      <p className="lt-tanggal" style={{ color: '#be185d', fontWeight: '500' }}>
                        Jadwal Wawancara: {new Date(app.created_at).toLocaleDateString('id-ID')}, 10:00 WIB
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="lt-progress">
                    {stepLabels.map((label, i) => (
                      <div key={i} className="lt-step-wrap">
                        <div className={`lt-step-circle ${i <= 2 ? 'lt-step-aktif' : ''}`}>
                          {i <= 2 ? (
                        ...
                        <span className={`lt-step-label ${i <= 2 ? 'lt-label-aktif' : ''}`}>{label}</span>
                        {i < stepLabels.length - 1 && (
                          <div className={`lt-step-line ${i < 2 ? 'lt-line-aktif' : ''}`} />
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="lt-card-right">
                    <span className="ls-badge ls-badge-wawancara">
