  ...
  return (
    <div className={cardCls}>
      <div className="dpl-card-header" style={{ flex: isFullWidth ? 1 : undefined }}>
        {app.user?.avatar ? (
          <img src={`https://jobportal-api-zebb.onrender.com/storage/${app.user.avatar}`} alt={app.user?.name} className="dpl-avatar" style={{ objectFit: 'cover' }} />
        ) : (
...
function ProsesModal({ candidate, onClose, onSave }) {
  ...
  const handleSimpan = async () => {
    ...
    await fetch(`https://jobportal-api-zebb.onrender.com/api/applications/${candidate.id}/status`, {
      method: 'PUT',
    ...
    <div className="dpl-modal-content">
      ...
      <div className="dpl-proses-kandidat">
        {candidate.user?.avatar ? (
          <img src={`https://jobportal-api-zebb.onrender.com/storage/${candidate.user.avatar}`} alt={candidate.user?.name} className="dpl-modal-avatar big" style={{ objectFit: 'cover', width: 40, height: 40 }} />
        ) : (
        ...
        {type === 'profil' && candidate && (
          <div className="dpl-modal-content">
            ...
            <div className="dpl-modal-profile-header">
              {candidate.user?.avatar ? (
                <img src={`https://jobportal-api-zebb.onrender.com/storage/${candidate.user.avatar}`} alt={candidate.user?.name} className="dpl-modal-avatar big" style={{ objectFit: 'cover' }} />
              ) : (
...
export default function DataPelamar() {
  ...
  const fetchData = () => {
    setLoading(true);
    fetch('https://jobportal-api-zebb.onrender.com/api/my-jobs', { headers: { 'Authorization': `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="dpl-root">
      ...
      <aside className={`dpl-sidebar ${isSidebarOpen ? "sidebar-open" : ""}`}>
